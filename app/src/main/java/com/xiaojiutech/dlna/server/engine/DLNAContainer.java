package com.xiaojiutech.dlna.server.engine;



import android.util.Log;

import com.xiaojiutech.dlna.utils.DLNAUtil;
...
public class DLNAContainer {
	...
	public synchronized void addDevice(Device d) {
		...
		mDevices.add(d);
		Log.d(TAG, "Devices add a device" + d.getDeviceType());
	...
	public synchronized void removeDevice(Device d) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (d.getUDN().equalsIgnoreCase(udnString)) {
				...
				Log.d(TAG, "Devices remove a device");
